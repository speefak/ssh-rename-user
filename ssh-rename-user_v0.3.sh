...
# Hinweis: read -i funktioniert nur mit -e zusammen (editierbar machen)
# Bei Passwort (-s) wird der Default-Wert trotzdem nicht angezeigt!

echo "Drücke Enter, um den voreingestellten Wert zu übernehmen."
echo

read -e -i "$DEFAULT_HOST"   -p "Ziel-Host (IP oder Domain)     : " HOST
read -e -i "$DEFAULT_OLD_USER" -p "Alter Username                 : " OLD_USER
read -e -i "$DEFAULT_NEW_USER" -p "Neuer Username                  : " NEW_USER

# Passwort-Feld: Default wird angezeigt, aber nicht sichtbar (nur editierbar)
# Wenn du Enter drückst → DEFAULT_OLD_PASS wird verwendet
read -e -i "$DEFAULT_OLD_PASS" -p "Passwort für $OLD_USER (sudo-fähig): " OLD_PASS
echo
...
sshpass -p "$TMP_ROOT_PASS" ssh -t -o StrictHostKeyChecking=no "root@$HOST" << EOF || { echo "Root-Login fehlgeschlagen – Root bleibt aktiviert!"; exit 1; }
  ...
  # Lösche die temporäre Zeilen – z. B. alles ab dem Marker bis zur nächsten Leerzeile oder einfach die ersten 2 Zeilen
