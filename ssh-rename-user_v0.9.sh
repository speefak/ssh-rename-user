...
sshpass -p "$TMP_ROOT_PASS" ssh -q -t -o StrictHostKeyChecking=no "root@$HOST" "bash -s" << EOF || { echo "Root login failed – root access remains enabled!"; exit 1; }
  ...
  echo "→ Terminating all sessions and processes of $OLD_USER ..."

  # 1. Clean termination of all sessions via logind (most important!)
  loginctl terminate-user "$OLD_USER" 2>/dev/null || true
  sleep 2

  # 2. Explicitly kill remaining processes (including systemd --user)
  loginctl kill-user "$OLD_USER" -s SIGTERM 2>/dev/null || true
  sleep 1

  # 3. If something is still hanging → force hard kill
  if ps -u "$OLD_USER" -o pid= | grep -q . ; then
      echo "→ Some processes still alive → forcing SIGKILL ..."
      loginctl kill-user "$OLD_USER" -s SIGKILL 2>/dev/null || true
      pkill -u "$OLD_USER" -9 || true
      sleep 1
  fi

  # 4. Explicitly stop systemd --user instance (sometimes necessary)
  machinectl shell "$OLD_USER"@"$(hostname)" -- systemctl --user stop '*' 2>/dev/null || true
  # alternative:
  # sudo -u "$OLD_USER" systemctl --user stop '*' 2>/dev/null || true

  # 5. Final check
  sleep 1
  if ps -u "$OLD_USER" -o pid= | grep -q . ; then
      echo "ERROR: Could not kill all processes of $OLD_USER!"
      echo "Remaining processes:"
      ps -u "$OLD_USER" -o pid,ppid,cmd --no-headers
      echo "Rename will most likely fail. Aborting."
      exit 1
  else
      echo "→ All processes of $OLD_USER terminated successfully."
  fi
    
  echo "→ Changing username from $OLD_USER to $NEW_USER (full consistent rename) ..."

  groupmod -n "$NEW_USER" "$OLD_USER"
  usermod -l "$NEW_USER" -d "/home/$NEW_USER" -m -c "$NEW_USER" "$OLD_USER"
