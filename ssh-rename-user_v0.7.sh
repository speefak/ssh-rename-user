...
# check ssh old user login on
echo -n "→ Testing SSH login with old user ... "
...
sshpass -p "$OLD_PASS" ssh -q -t -o StrictHostKeyChecking=no "$OLD_USER@$HOST" << EOF || { echo "Failed to set root password!"; exit 1; }
...
sshpass -p "$OLD_PASS" ssh -q -t -o StrictHostKeyChecking=no "$OLD_USER@$HOST" << EOF || { echo "Failed to enable root SSH login!"; exit 1; }
...
sshpass -p "$TMP_ROOT_PASS" ssh -q -t -o StrictHostKeyChecking=no "root@$HOST" << EOF || { echo "Root login failed – root access remains enabled!"; exit 1; }
